...
- `verify_protocol_finality`'s live RPC path is verified end-to-end on this deployment: a real signed call reached the official RPC and every consensus node independently derived the identical fail-closed result for the deployment's own real, finalized, non-incident transaction (see `docs/DEPLOYMENT.md`, "Live release proof").
