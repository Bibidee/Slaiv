...
- v3 Studionet contract (pending merge): `0xE2c8ECFa29Dd67a1dDe8026Df62628bE765d78A5` -- see `docs/DEPLOYMENT.md` for the full record, source hash, source-match caveat, and live proof. Supports `MISSED_EXECUTION_WINDOW` only; `MISSED_APPEAL_WINDOW` was found to be validator-unbound in an earlier audit deployment and was removed (see `docs/DEPLOYMENT.md`, "Why the prior deployments were superseded").
- Full live-transaction test table (11 transactions against the current release, every row mapped to what it tested and whether the outcome matched): `docs/DEPLOYMENT.md`, section "Live release proof (v3, permissionless -- current release)". 4 succeeded (deploy, policy creation, claim filing, permissionless evidence); 7 were deliberate attack/misuse attempts (impersonation, duplicate policy, unsupported-event coverage, unauthorized evidence, out-of-sequence state transitions) that correctly failed -- an `ERROR` badge on those rows means the security model worked, not that something broke.
