...
Development is isolated on the `permissionless-v3` branch (draft PR #5, not merged to `main`). A permissionless v3 contract has been deployed to Studionet, but the public frontend still points at the historical authority-gated release below until this branch is reviewed and merged.

- v3 Studionet contract (pending merge): `0xc17bfE775D46080E9A58F1eC80edC2E7A04DF101` -- see `docs/DEPLOYMENT.md` for the full record, source hash (`SOURCE MATCH: PASS`, `PREFLIGHT: PASS`), and live proof. Supports `MISSED_EXECUTION_WINDOW` only; `MISSED_APPEAL_WINDOW` was found to be validator-unbound in an earlier audit deployment and was removed (see `docs/DEPLOYMENT.md`, "Why the prior deployments were superseded").
- `verify_protocol_finality`'s live RPC path is now verified end-to-end: a real signed call reached `gl.nondet.web.post()`, queried the official RPC, and every consensus node independently derived the identical fail-closed result for a real non-incident transaction (see `docs/DEPLOYMENT.md`, "verify_protocol_finality live RPC path: verified"). That test also surfaced and fixed a consensus-efficiency bug in how validators vote agreement (see "Consensus validator() agreement fix" in the same file).
- Full live-transaction test tables (every row mapped to what it tested and whether the outcome matched): `docs/DEPLOYMENT.md`, section "Live release proof (v3, permissionless -- current release)". SUCCESS rows are legitimate policy/claim/evidence actions; ERROR rows are deliberate attack/misuse attempts (impersonation, duplicate policy, unsupported-event coverage) that correctly failed -- an `ERROR` badge on those rows means the security model worked, not that something broke.
