...
| `testnetAsimov` | `https://rpc-asimov.genlayer.com` | **Not verified.** The RPC URL is real (from GenLayer's own published `genlayer-js` SDK chain config) and responds to `eth_getTransactionByHash`, but this codebase has not confirmed a real Asimov transaction returns the same enriched consensus fields Studio's node does — the one sample checked returned only a bare hash with every consensus field null. |
| `testnetBradbury` | `https://rpc-bradbury.genlayer.com` | **Not verified**, same reasoning as Asimov. |

`SUBJECT_NETWORKS` and `create_policy`'s subject-network check are both derived directly from `RPC_ENDPOINTS`, so a policy can no longer be created for a network SLAIV cannot actually verify: `create_policy` reverts with `"invalid policy subject"` for `testnetAsimov`/`testnetBradbury` today. This closes a gap in an earlier revision, where policies for those networks could be created (and immutably promise coverage) even though `verify_protocol_finality` would fail closed for every claim filed against them ("network verification source not available"). Asimov/Bradbury support can be added later, but only by confirming their RPC response shape and adding an entry to `RPC_ENDPOINTS` -- at which point `SUBJECT_NETWORKS` and policy creation pick it up automatically.
