...
- Outsider-triggered `verify_protocol_finality` against a real, finalized, non-incident Studionet transaction (this deployment's own tx `0x26267d45384a2879226f59...caf833`): rejected (`contract_error`, "protocol finality not verified"). Claim state remained `AWAITING_FINALITY` / `underlying_finality=PENDING` -- no synthetic protocol fact was fabricated to force a positive result.
...
- `get_protocol_stats` after the run: `{"policy_count": 1, "claim_count": 1, "permissionless": true}` -- only the two legitimate records exist; no impersonation or duplicate records were created.

No genuine validator-timeout incident was available during this test, so the positive settlement path (`UNDER_REVIEW` -> judgment -> appeal -> finalize) was not exercised end-to-end on-chain. This mirrors the same limitation recorded for the historical v2 release below and is not a new gap.
