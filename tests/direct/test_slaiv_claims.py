...
def test_evidence_is_persisted_and_rotated_adapter_promotes_finality(direct_vm,direct_deploy,direct_alice,direct_bob,direct_charlie):
    ...
    e={"kind":"CLAIMANT_ASSERTION","source":"claimant","reference":"claimant-ref","commitment":"e1","finality":"UNVERIFIED"}; c.append_evidence('clm_beta',e,'e1')
...
def test_full_review_appeal_and_finalization_lifecycle(direct_vm,direct_deploy,direct_alice):
    ...
    c.append_evidence('clm_full',{'kind':'CLAIMANT_ASSERTION','source':'test claimant','reference':'test://claimant','commitment':'claimant-full','finality':'UNVERIFIED'},'claimant-full')
    protocol={'kind':'PROTOCOL_FACT','protocol':'genlayer','validator':'validator-1','claim_id':'clm_full','source':'GENLAYER_STAKING_ADAPTER','reference':'https://evidence.example/full-record','observed_at_ts':2,'source_record_sha256':'b'*64,'finality':'FINAL'}
    c.record_protocol_finality('clm_full',protocol)
    verdict={'eligibility':'PARTIALLY_APPROVED','incident_class':'MISSED_EXECUTION_WINDOW','slash_final':True,'covered_event':True,'exclusion_triggered':False,'eligible_loss':80,'confidence':1,'evidence_findings':'test','policy_findings':'test','reasoning_summary':'test'}
    ...
    c.record_appeal('clm_full','Material evidence shows the documented loss was complete.','test://new-evidence')
    direct_vm.clear_mocks(); direct_vm.mock_llm(r'.*Return JSON only with result.*',json.dumps({'result':'OVERTURN','eligible_loss':100}))
