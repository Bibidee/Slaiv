...
class SlaivClaims(gl.Contract):
    ...
    def review_appeal(self, claim_id: str) -> None:
        ...
        if not isinstance(result.get("eligible_loss"), int) or result["eligible_loss"] < 0 or result["eligible_loss"] > c["documented_loss"]: raise Exception("invalid appeal amount")
        effective=dict(original)
        if result["result"] in ("MODIFY","OVERTURN"):
            effective["eligible_loss"]=result["eligible_loss"]; effective["eligibility"]="PARTIALLY_APPROVED" if effective["eligible_loss"]<c["documented_loss"] else "APPROVED"; self._store(self.effective_reviews,claim_id,effective); c["state"]=effective["eligibility"]
