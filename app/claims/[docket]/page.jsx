...
function PublicEvidenceForm({claim,run}){return <details><summary>Add public-source evidence</summary><form onSubmit={async event=>{event.preventDefault();const evidence=await evidenceFrom(event.currentTarget,claim.claim_id,'PUBLIC_SOURCE');await run('append_evidence',[claim.claim_id,evidence,evidence.content_hash])}}><p className="form-note">Any wallet may bind a public source to the open dossier. Public evidence is not protocol finality and cannot by itself unlock judgment.</p><EvidenceFields/><button className="button">Add public exhibit</button></form></details>}
function FinalityForm({claim,policy,run}){return <details open><summary>Verify protocol event</summary><form onSubmit={async event=>{event.preventDefault();const data=new FormData(event.currentTarget);await run('verify_protocol_finality',[claim.claim_id,String(data.get('eventId')||'').trim(),String(data.get('reference')||'').trim()])}}><p className="form-note">Anyone can submit a candidate event. The caller does not set finality or the outcome: GenLayer validators independently read the official source and must agree on the protocol facts.</p><label>GenLayer event / transaction ID<input name="eventId" placeholder="0x…" pattern="0x[0-9a-fA-F]{64}" required/></label><label>Official GenLayer explorer record<input name="reference" type="url" placeholder={`https://explorer-${policy.subject_network==='studionet'?'studio':policy.subject_network==='testnetAsimov'?'asimov':'bradbury'}.genlayer.com/tx/0x…`} required maxLength="2000"/></label><button className="button">Verify with GenLayer consensus</button></form></details>}

function ActionPanel({claim,policy,isClaimant,connected,run,tx}){
...
export default function Case(){
  ...
  return <main className="app-page"><PageHead eyebrow={docket(claim.claim_id)} title="Adjudication docket"><div className="schedule-head"><Stamp state={stampFor(claim.state,payout)}/><span>{claim.state.replaceAll('_',' ')}</span></div></PageHead>
    ...
    <section className="case-layout"><article>
      ...
      <ActionPanel claim={claim} policy={policy} isClaimant={isClaimant} connected={Boolean(account)} run={run} tx={transaction.tx}/>
