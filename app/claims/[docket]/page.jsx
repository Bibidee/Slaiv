...
export default function Case(){
  ...
  return <main className="app-page"><PageHead eyebrow={docket(claim.claim_id)} title="Adjudication docket"><div className="schedule-head"><Stamp state={stampFor(claim.state,payout)}/><span>{claim.state.replaceAll('_',' ')}</span></div></PageHead>
    ...
    </article><aside className="exhibits"><p className="eyebrow">EXHIBITS</p>{evidence.map((item,index)=><div className="exhibit" key={item.evidence_id}><code>EX-{String.fromCharCode(65+index)} · {item.evidence_id}</code><Stamp state={item.kind==='PROTOCOL_FACT'?'SETTLED':'BOUND'}/><b>{item.kind.replaceAll('_',' ')}</b><span>{item.source}</span><a href={item.reference} target="_blank" rel="noreferrer">Source record ↗</a><small>{fmtDate(item.submitted_at)} · {item.content_hash} · {item.submitter||'submitter unavailable'}</small></div>)}{!evidence.length?<p className="empty-copy">No exhibits recorded.</p>:null}
      <section className="policy-mini"><p className="eyebrow">DETERMINISTIC POLICY BOUNDARY</p><span>Validator<b>{policy.validator}</b></span><span>Coverage period<b>{fmtDate(policy.coverage_start_ts)} — {fmtDate(policy.coverage_end_ts)}</b></span><span>Covered events<b>{(policy.covered_events||[]).join(', ')}</b></span><span>Exclusions<b>{(policy.exclusions||[]).join(', ')||'None recorded'}</b></span><span>Coverage limit<b>{policy.coverage_limit} GEN</b></span><span>Deductible<b>{Number(policy.deductible_bps)/100}%</b></span></section>{claim.state==='FINAL'?<Link className="button" href={`/claims/${claim.claim_id}/settlement`}>View settlement</Link>:null}
    </aside></section>
  </main>;
}
