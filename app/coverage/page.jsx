  ...
  return <main className="app-page">
    ...
    <div className="market">
      ...
      <section className="docket-grid">
        {live.loading?<p className="loading">Reading contract ledger…</p>:live.error?<div className="read-error">{live.error}<button onClick={()=>void live.refresh()}>Retry read</button></div>:live.data?.length?live.data.map(policy=><Link key={policy.policy_id} href={`/coverage/${encodeURIComponent(policy.policy_id)}`}><Docket id={docket(policy.policy_id)} state="BOUND"><p className="case-label">BOUND POLICY SCHEDULE</p><h2>{policy.validator}</h2><div className="policy-facts"><span>Window <b>{fmtDate(policy.coverage_start_ts)} — {fmtDate(policy.coverage_end_ts)}</b></span><span>Limit <b>{policy.coverage_limit} GEN</b></span><span>Deductible <b>{Number(policy.deductible_bps)/100}%</b></span></div></Docket></Link>):<Empty href="/coverage/new" label="Bind the first policy →">No live coverage is currently bound on this contract.</Empty>}
      </section>
    </div>
  </main>;
}
