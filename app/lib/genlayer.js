...
export async function claimDossier(id){const claim=parse(await read('get_claim',[id]),null);if(!claim)return null;const [policy,evidence,review,effectiveReview,payout]=await Promise.all([read('get_policy',[claim.policy_id]),read('get_evidence',[id]),read('get_review',[id]),read('get_effective_review',[id]),read('get_payout',[id])]);return {claim,policy:parse(policy,{}),evidence:parse(evidence,[]),review:parse(review,null),effectiveReview:parse(effectiveReview,null),payout:String(payout)}}
