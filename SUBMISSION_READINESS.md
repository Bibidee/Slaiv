...
A full manual multi-wallet Studionet lifecycle test was run directly against the current release address, `0xC8DC9930efA7276280E2E23BC59038420e97F747`, covering: legitimate policy/claim/evidence flows, duplicate policy rejection, outsider policy/claim impersonation rejection, per-wallet public-evidence quota enforcement, and live proof of the two new hardening checks added this pass -- a malformed (non-hex) `policy_commitment` and a malicious link-local-metadata `PUBLIC_SOURCE` URL were both submitted and both confirmed rejected (absent from state afterward). All 13 transactions produced their expected result. Full transaction table: `docs/DEPLOYMENT.md`, "Live release proof (current release, `0xC8DC9930efA7276280E2E23BC59038420e97F747`)".

A `verify_protocol_finality` call was intentionally not attempted live in this run: the only reliable way to submit its hex `event_id` argument requires `scripts/verify-protocol-finality.mjs` with the user's own signing key, run in their own terminal -- see `docs/DEPLOYMENT.md` for why the CLI's `write --args` path corrupts this specific argument. The RPC path itself was previously verified live (identical contract logic) on a now-superseded deployment; it has not yet been re-verified against the current address.

The fetch-crowding-resistance and appeal-evidence-canonicalization behaviors added this pass require a `DENIED`/`PARTIALLY_APPROVED` claim state and an LLM judgment call to exercise, which in turn requires a genuine protocol incident this session did not have -- these remain proven only by Direct Mode's synthetic fixtures (91/91 passing), not a live run.
